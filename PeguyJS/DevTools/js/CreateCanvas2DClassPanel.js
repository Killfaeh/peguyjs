function CreateCanvas2DClassPanel()
{
	////////////////
	// Attributes //
	////////////////
	...
	var html = '<div class="createClassPanel" >'
					...
					+ '<div id="settingsPanel" class="settingsPanel" >'
						+ '<table>'
							+ '<td colspan="3" >'
								+ '<h4><span id="classLabel" >' + "Canvas 2D class" + '</span><span id="docIcon" class="docIcon" ></span></h4>'
							+ '</td>'
							+ '<tr>'
	...
	this.loadFromJSON = function($json)
	{
		...
		if (utils.isset($json.attributes))
		{
			...
			{
				var attributeInput = new Attribute2DInput();
	...
	{
		code = '';
		var constructorParams = '$x, $y, $angle';
		...
		for (var i = 0; i < attributes.length; i++)
		{
			...
			if (name !== '')
			{
				...
				if (/2D$/.test(type))
				{
					...
					if (type === 'LinearGradient2D')
						emptyValue = '$colors, $x1, $y1, $x2, $y2';
					else if (type === 'RadialGradient2D')
						emptyValue = '$colors, $startCenterX, $startCenterY, $startRadius, $endCenterX, $endCenterY, $endRadius';
					else if (type === 'ConicGradient2D')
						emptyValue = '$colors, $startAngle, $x, $y';
					else if (type === 'Image2D')
						emptyValue = '$sourceImage, $sourceX, $sourceY, $sourceWidth, $sourceHeight, $targetX, $targetY, $targetWidth, $targetHeight';
					else if (type === 'Sprite2D')
						emptyValue = '$sourceImage, $nbAnimations, $nbFrames, $vertical, $targetX, $targetY, $targetWidth, $targetHeight';
					else if (type === 'Rect2D')
						emptyValue = '$x, $y, $width, $height';
					
					if (defaultValue === '')
						attributesDeclaration = attributesDeclaration + '	var ' + name + ' = new ' + type + '(' + emptyValue + ');\n';
					else
						attributesDeclaration = attributesDeclaration + '	var ' + name + ' = new ' + type + '(' + defaultValue + ');\n';
					
					if (component.getById('extend').value === 'group2D')
						addToGroup = addToGroup + '	group2D.addObject(' + name + ');\n';
				}
				else
				{
		...
		if (component.getById('extend').value === 'object2D')
		{
			methodsDeclaration = methodsDeclaration + '	this.render = function($context)\n';
			methodsDeclaration = methodsDeclaration + '	{\n';
			methodsDeclaration = methodsDeclaration + "		$this['super'].render($context);\n\n";
			methodsDeclaration = methodsDeclaration + '		console.log("Execute render...");\n\n';
			methodsDeclaration = methodsDeclaration + '		$context.restore();\n';
			methodsDeclaration = methodsDeclaration + '	};\n\n';
		}
		...
		if (component.getById('extend').value === 'group2D')
		{
			code = code + '	var group2D = new Group2D();\n';
			code = code + '	group2D.setX($x);\n';
			code = code + '	group2D.setY($y);\n';
			code = code + '	group2D.setAngle($angle);\n\n';
		}
		else
		{
			code = code + '	var object2D = new Object2D();\n';
			code = code + '	object2D.setX($x);\n';
			code = code + '	object2D.setY($y);\n';
			code = code + '	object2D.setAngle($angle);\n\n';
		}
		
		if (component.getById('extend').value === 'group2D')
			code = code + addToGroup + '\n';
		...
		if (component.getById('extend').value === 'group2D')
			code = code + '	var $this = utils.extend(group2D, this);\n';
		else
			code = code + '	var $this = utils.extend(object2D, this);\n';
	...
	component.getById('extend').onChange = function() { onChange(); };
	component.getById('extend').onchange = function() { onChange(); };
	...
	{
		console.log("CreateCanvas2DClassPanel : " + $event.keyCode);
	...
	this.getJSON = function()
	{
		var jsonObject = 
		{
			...
			extend: component.getById('extend').value,
			attributes: [],
			methods: [],
			doc: doc
		};
	...
	this.getHTMLdoc = function()
	{
		...
		if (component.getById('extend').value === 'group2D')
		{
			docCode = docCode + '		<tr>\n';
			docCode = docCode + '			<td>\n';
			docCode = docCode + '				<pre><code class="javascript" >var group2D</code></pre>\n';
			docCode = docCode + '			</td>\n';
			docCode = docCode + '			<td>Objet Group2D</td>\n';
			docCode = docCode + '			<td>\n';
			docCode = docCode + '				Objet <em class="inline-code" >Group2D</em> dont hérite <em class="inline-code" >' + component.getById('classNameInput').value + '</em>.\n';
			docCode = docCode + '			</td>\n';
			docCode = docCode + '		</tr>\n';
		}
		else
		{
			docCode = docCode + '		<tr>\n';
			docCode = docCode + '			<td>\n';
			docCode = docCode + '				<pre><code class="javascript" >var object2D</code></pre>\n';
			docCode = docCode + '			</td>\n';
			docCode = docCode + '			<td>Objet Object2D</td>\n';
			docCode = docCode + '			<td>\n';
			docCode = docCode + '				Objet <em class="inline-code" >Object2D</em> dont hérite <em class="inline-code" >' + component.getById('classNameInput').value + '</em>.\n';
			docCode = docCode + '			</td>\n';
			docCode = docCode + '		</tr>\n';
		}
		...
		if (component.getById('extend').value === 'object2D')
		{
			docCode = docCode + '		<tr>\n';
			docCode = docCode + '			<td>\n';
			docCode = docCode + '				<pre><code class="javascript" >this.render = function($context)</code></pre>\n';
			docCode = docCode + '			</td>\n';
			docCode = docCode + '			<td>Accès public</td>\n';
			docCode = docCode + '			<td>\n';
			docCode = docCode + '				Méthode de rendu sur le canevas auquel appartient le contexte <em class="inline-code" >$context</em>.\n';
			docCode = docCode + '			</td>\n';
			docCode = docCode + '		</tr>\n';
		}
...
if (Loader !== null && Loader !== undefined)
	Loader.hasLoaded("createCanvas2DClassPanel");